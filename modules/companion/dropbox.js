...
function get_path_last_days(how_many_days) {
  ...
  var prev_data_path = d_prev.getFullYear()
    +("0" + (parseInt(d_prev.getMonth())+1)).slice(-2)
    +"/"
    +("0" + d_prev.getDate()).slice(-2)
    +"/";
  console.log("get_path_last_days() = " + "/glucose/" + prev_data_path);
  return "/glucose/" + prev_data_path;
}

async function get_data_has_more(dropbox_token, dbx_cursor, yagi_patient_name, day_nr) {
  bgs_day[day_nr] = '';
  bgs = '';
  prev_datetime = 0;
  ...
  return await fetch(`https://api.dropboxapi.com/2/files/list_folder/continue`, {
    headers: {
    ...
    for(var prop in json_response) {
      ...
      for(var elem in item) { 
        ...
        if (item_sub[".tag"] == "file" && path_file_name != undefined) {
          ...
          if (row.bg_sensor != undefined 
              && row.data_type == 'GLU'
              && (yagi_patient_name==null 
                  || yagi_patient_name+'' == '' 
                  || (yagi_patient_name+'').toLowerCase() == (row.patient+'').toLowerCase())
              && row.datetime > prev_datetime //the datetime must be after the previous                                 
             ) {
            
            //Add points with no data every hole of 5 minutes
            for (var i=1; max_datetime > 0 && i < MAX_BG_FETCHED && i < (row.datetime - max_datetime) / (5 * 61 * 1000); i++) { //a little more then 5 minutes
              console.log('Added empty point: ' + row.datetime + '-' + max_datetime + '-'+((row.datetime - max_datetime) / (5 * 61 * 1000)));
            ...
            bgs_day[day_nr] = 
              ...
              + '"sgv":"' + row.bg_sensor 
              + '","trend":' + row.trend 
              + ',"direction":"' + row.arrow_value 
              + '","datetime":' + row.datetime + '%DELTA%'
              + '}' + ',' + bgs_day[day_nr];
            ...
            //console.log("discarded - " + row.datetime + " - " + prev_datetime)
          }
        } 
      }
    }
    ...
    if (bgs_day[day_nr].slice(-1) == ',') {
      bgs_day[day_nr] = bgs_day[day_nr].substring(0, bgs_day[day_nr].length-1)      
    } 
    
    //At the first occurrence I also add the delta  
    bgs_day[day_nr] = bgs_day[day_nr].replace('%DELTA%', ', "bgdelta":' + delta);
...
export default class dropbox {  
    async getData(dropbox_token, yagi_patient_name) {
      ...
      ret_json_dummy ={bgs:[{sgv:'120',bgdelta:0,iob:0,cob:0,datetime:(new Date()).getTime(),direction:'warning',currentbg:'',rawbg:'',tempbasal:'',loopstatus:'',},{sgv:'120'},{sgv:'120'},{sgv:'120'},{sgv:'120'},{sgv:'120'},{sgv:'120'},{sgv:'120'},{sgv:'120'},{sgv:'120'},{sgv:'120'},{sgv:'120'},{sgv:'120'},{sgv:'120'},{sgv:'120'},{sgv:'120'},{sgv:'120'},{sgv:'120'},{sgv:'120'},{sgv:'120'},{sgv:'120'},{sgv:'120'},{sgv:'120'},{sgv:'120'},{sgv:'120'},{sgv:'120'},{sgv:'120'},{sgv:'120'},{sgv:'120'},{sgv:'120'},{sgv:'120'},{sgv:'120'},{sgv:'120'},{sgv:'120'},{sgv:'120'},{sgv:'120'},{sgv:'120'},{sgv:'120'},{sgv:'120'},{sgv:'120'},{sgv:'120'},{sgv:'120'},{sgv:'120'},{sgv:'120'},{sgv:'120'},{sgv:'120'},{sgv:'120'}]};

	  /*
      if (1==0 && (new Date()).getTime() - this.last_fetch_datetime < 10000) { //less than 10 seconds
        console.log("Workaround: second call rejected!")
	  ...
	  */
      ...
      if (object_count < MAX_BG_FETCHED) {
        console.log("Reading previous day...")
      ...
      if (bgs_day[0].length > 0 && bgs_day[1].length == 0) {
         bgs = bgs_day[0]
         console.log("Read only day 0")
      } else if (bgs_day[0].length > 0 && bgs_day[1].length > 0) {
         bgs = bgs_day[0] + ',' + bgs_day[1]
         console.log("Read day 0+1")
      } else if (bgs_day[0].length == 0 && bgs_day[1].length > 0) {
         bgs = bgs_day[1]
         console.log("Read only day 1")
      } else {
         return ret_json_dummy;
      }

      let currentTime = new Date()
      let ret_json = JSON.parse('{"status":[{"now":'+max_datetime+'}],"bgs":[' + bgs + '],"cals":[]}');
      return ret_json
    }
};