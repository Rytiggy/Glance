...
export default class settings { 
 get(dataReceivedFromWatch) {
   ...
   if (settingsStorage.getItem('dataSource')) {
     ...
     settingsStorage.setItem("dataSource", JSON.stringify({"selected":[0],"values":[{"name":"Dexcom","value":"dexcom"}]}));
     dataSource = 'dexcom';
   }   
 ...
     // local spike addr for my comp
   ...
   if (settingsStorage.getItem('timeFormat')) {
     ...
     timeFormat = '12hr';
     settingsStorage.setItem("timeFormat", JSON.stringify({"selected":[0],"values":[{"name":timeFormat,"value":false}]}));
   }
   ...
   if (settingsStorage.getItem('tempType')) {
     ...
     tempType = 'f';
     settingsStorage.setItem("tempType", JSON.stringify({"selected":[0],"values":[{"name":'Fahrenheit',"value":tempType}]}));
   } 
   ...
   if (settingsStorage.getItem('bgColor')) {
     ...
     if(bgColor === '#FFFFFF') {
       ...
       if (!saveColor) {
         settingsStorage.setItem("hexColor", JSON.stringify({"name":validateHexCode(bgColor)}));  
         settingsStorage.setItem("hexColorTwo", JSON.stringify({"name":validateHexCode(bgColorTwo)}));  
       } else {
   ...
   if (settingsStorage.getItem('textColor')) {
    ...
    settingsStorage.setItem("textColor", JSON.stringify({"name":validateHexCode(textColor)})); 
   } else if (!textColor) {
    textColor = '#ffffff';
